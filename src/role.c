...
static PyObject *HC_Role_load_image(HC_Role *self, PyObject *args,
                                    PyObject *kwds) {
  char *path;
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "s", (char *[]){"path", NULL},
                                   &path)) {
    return NULL;
  }
  ...
  PyList_Append(self->costume_list,
                PyCapsule_New(surface, "SDL_Surface*", NULL));
  ...
  if (PyLong_AsLong(self->costume_number) == -1) {
    ...
    if (!result) {
      PyErr_SetString(PyExc_RuntimeError,
                      "Fail to add numbers at Role.load_image");
      Py_XDECREF(one);
      Py_XDECREF(result);
      Py_RETURN_FALSE;
    }
...
static PyObject *HC_Role_load_image_from_surface(HC_Role *self, PyObject *args,
                                                 PyObject *kwds) {
  PyObject *py_object;
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", (char *[]){"surface", NULL},
                                   &py_object)) {
    return NULL;
  }
  if (!PyObject_TypeCheck(py_object, &HC_SurfaceType)) {
    PyErr_SetString(
        PyExc_TypeError,
        "Value of Role.load_image_from_surface must be a HC_Surface.");
    return NULL;
  }
  HC_Surface *hc_surface = (HC_Surface *)py_object;
  PyList_Append(self->costume_list,
                PyCapsule_New(hc_surface->surface, "SDL_Surface*", NULL));
...
static PyObject *HC_Role_collide(HC_Role *self, PyObject *args,
                                 PyObject *kwds) {
  ...
  // 边界检查
  if ((x1 < x2 && x1 + (double)a->w < x2) ||
      (x1 > x2 && x1 > x2 + (double)b->w) ||
      (y1 > y2 && y1 > y2 + (double)b->h) ||
      (y1 < y2 && y1 + (double)a->h < y2)) {
  ...
  const char *start1 =
      (char *)a->pixels + max(0, (int)(y2 - y1)) * a->pitch + left1 * 4;
  const char *start2 =
      (char *)b->pixels + max(0, (int)(y1 - y2)) * b->pitch + left2 * 4;
  ...
  for (int line = 0; line < height; line++) {
    for (int i = 0; i < width; i++) {
      memcpy(&pixel_a, start1 + i * 4, 4);
      memcpy(&pixel_b, start2 + i * 4, 4);
      // 提取透明通道
  ...
  for (int i = 0; i < PyList_Size(self->costume_list); i++) {
    ...
    if (temp != NULL && PyCapsule_CheckExact(temp)) {
      SDL_Texture *temp_texture = SDL_CreateTextureFromSurface(
          display->renderer, PyCapsule_GetPointer(temp, "SDL_Surface*"));
      PyList_Append(self->buffer_list,
                    PyCapsule_New(temp_texture, "SDL_Texture*", NULL));
    } else {
...
static int HC_Role_set_flip_x(HC_Role *self, PyObject *new_value,
                              void *closure) {
...
static int HC_Role_set_flip_y(HC_Role *self, PyObject *new_value,
                              void *closure) {
  if (!PyBool_Check(new_value)) {
...
static int HC_Role_set_facing_angle(HC_Role *self, PyObject *new_value,
                                    void *closure) {
  ...
  PyErr_SetString(PyExc_TypeError,
                  "Role.facing_angle value must be a float or a int.");
  return -1;
}
...
static int HC_Role_set_position(HC_Role *self, PyObject *new_value,
                                void *closure) {
...
static PyGetSetDef HC_Role_getSetDef[] = {
    {"flip_x", (getter)HC_Role_get_flip_x, (setter)HC_Role_set_flip_x,
     "set or get Role.flip_x."},
    {"flip_y", (getter)HC_Role_get_flip_y, (setter)HC_Role_set_flip_y,
     "set or get Role.flip_y."},
    {"facing_angle", (getter)HC_Role_get_facing_angle,
     (setter)HC_Role_set_facing_angle, "set or get Role.facing_angle."},
    {"position", (getter)HC_Role_get_position, (setter)HC_Role_set_position,
     "set or get Role.position."},
    {"costume_number", (getter)HC_Role_get_costume_number,
     (setter)HC_Role_set_costume_number, "set or get Role.costume_number."},
    {NULL}};

static PyMemberDef HC_RoleMembers[] = {
    {"costume_list", T_OBJECT, offsetof(HC_Role, costume_list), READONLY,
     "costume_list of CRole"},
    {"alpha", T_OBJECT, offsetof(HC_Role, alpha), 0, "alpha of CRole"},
    {"scale", T_OBJECT, offsetof(HC_Role, scale), 0, "scale of CRole"},
    {"width_scale", T_OBJECT, offsetof(HC_Role, width_scale), 0,
     "width_scale of CRole"},
    {"height_scale", T_OBJECT, offsetof(HC_Role, height_scale), 0,
     "height_scale of CRole"},
    {NULL, 0, 0, 0, NULL}};

static PyMethodDef HC_RoleMethodDef[] = {
    {"load_image", (PyCFunction)HC_Role_load_image,
     METH_KEYWORDS | METH_VARARGS, "Load image from path."},
    {"load_image_from_surface", (PyCFunction)HC_Role_load_image_from_surface,
     METH_KEYWORDS | METH_VARARGS, "Load image from a surface."},
    {"buffer", (PyCFunction)HC_Role_buffer, METH_NOARGS, "buffer all surface."},
    {"collide", (PyCFunction)HC_Role_collide, METH_KEYWORDS | METH_VARARGS,
     "collide with another role."},
    {NULL, NULL, 0, NULL}};
PyTypeObject HC_RoleType = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "HCGame.Role",
    .tp_basicsize = sizeof(HC_Role),
    .tp_itemsize = 0,
    .tp_flags = Py_TPFLAGS_DEFAULT,
    .tp_new =
        PyType_GenericNew, // 创建对象（分配内存），我这里填默认行为，通常不用动
    .tp_init = (initproc)HC_Role_init, // 初始化 __init__
    .tp_dealloc =
        (destructor)HC_Role_dealloc, // 销毁 __del__，无循环引用不需要GC介入
    .tp_doc = "CRole",
